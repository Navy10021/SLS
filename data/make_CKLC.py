...
for row in data_dict['judgment_contents']:
    ...
    if re.search('【주    문】(.+?)【이    유】', row):
        found_1 = "[주 문]" + re.search('【주    문】(.+?)【이    유】', row).group(1)
    if re.search('【이    유】(.+?)(주심)', row):
        found_2 = " [이 유]" + re.search('【이    유】(.+?)(주심)', row).group(1)
...
####################
CKLC = {}
CKLC['issue'] = issue_list
CKLC['summary'] = summary_list
CKLC['contents'] = contents_list

with open('CKLC.pickle', 'wb') as f:
    pickle.dump(CKLC, f, pickle.HIGHEST_PROTOCOL)
    

####################
## 5. Pickle Load ##
####################
with open('CKLC.pickle', 'rb') as f:
    data = pickle.load(f)

print(">> Corpus-1 Size :", len(data['issue']))
print(">> Corpus-2 Size :", len(data['summary']))
print(">> Corpus-3 Size :", len(data['contents']))
print(">> Total Number of Corpus : {}".format(len(data['issue'])+len(data['summary'])+len(data['contents'])))